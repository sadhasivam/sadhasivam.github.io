...
async function exportMarkdown(): Promise<void> {
  ...
  try {
    ...
    const name = document.querySelector('h1')?.textContent || 'Sadhasivam Jayabalaganesan';
    const subtitle = document.querySelector('h1 + p')?.textContent || 'Technology Leader | Platform Strategy & Innovation';

    // Extract contact info
    const contactSection = document.querySelector('.border-t.border-gray-300.pt-2');
    let contactInfo = '';
    if (contactSection) {
      const spans = contactSection.querySelectorAll('span:not(.print\\:hidden)');
    ...
    if (mainContent) {
      ...
      htmlToConvert = htmlToConvert.replace(/<span class="absolute -ml-4 text-indigo-500">•<\/span>/g, '');
      htmlToConvert = htmlToConvert.replace(/<span class="absolute -ml-4 text-indigo-500">&#x2022;<\/span>/g, '');
    }
